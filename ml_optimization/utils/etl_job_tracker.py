...
class ETLJobTracker:
    ...
    def ensure_table_exists(self):
        ...
        with self._get_connection() as conn:
            ...
            """)
            # Add check constraint for active_status if not present (PostgreSQL: add constraint only if missing)
            cursor.execute("""
                DO $$
                BEGIN
                    IF NOT EXISTS (
    ...
    def start_job(
        ...
        with self._get_connection() as conn:
            ...
            row = cursor.fetchone()
            if row:
                job_id, existing_cron = row
                # Optionally backfill cron_pattern if provided and not already set
                if cron_pattern and not existing_cron:
                    cursor.execute(
                        """
                ...
                cursor.execute(
                    ...
                    VALUES (%s, %s, %s, %s, %s, 'A')
                    """,
                    (job_id, job_name, job_type, tables_value, cron_pattern),
                )
